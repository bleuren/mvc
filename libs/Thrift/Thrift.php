...
abstract class TBase
{
    ...
    private function _readMap(&$var, $spec, $input)
    {
        ...
        $vtype = $spec['vtype'];
        $kread = $vread = null;
        if (isset(self::$tmethod[$ktype])) {
            $kread = 'read'.self::$tmethod[$ktype];
        } else {
            $kspec = $spec['key'];
        }
        if (isset(self::$tmethod[$vtype])) {
            $vread = 'read'.self::$tmethod[$vtype];
        } else {
            $vspec = $spec['val'];
        }
        $var = array();
    ...
    private function _readList(&$var, $spec, $input, $set = false)
    {
        ...
        $etype = $spec['etype'];
        $eread = $vread = null;
        if (isset(self::$tmethod[$etype])) {
            $eread = 'read'.self::$tmethod[$etype];
        } else {
            $espec = $spec['elem'];
        }
        $var = array();
    ...
    protected function _read($class, $spec, $input)
    {
        ...
        while (true) {
            ...
            if (isset($spec[$fid])) {
                ...
                if ($ftype == $fspec['type']) {
                    $xfer = 0;
                    if (isset(self::$tmethod[$ftype])) {
                        $func = 'read'.self::$tmethod[$ftype];
                        $xfer += $input->$func($this->$var);
                    } else {
                        switch ($ftype) {
            case TType::STRUCT:
    ...
    private function _writeMap($var, $spec, $output)
    {
        ...
        $kwrite = $vwrite = null;
        if (isset(self::$tmethod[$ktype])) {
            $kwrite = 'write'.self::$tmethod[$ktype];
        } else {
            $kspec = $spec['key'];
        }
        if (isset(self::$tmethod[$vtype])) {
            $vwrite = 'write'.self::$tmethod[$vtype];
        } else {
            $vspec = $spec['val'];
        }
        $xfer += $output->writeMapBegin($ktype, $vtype, count($var));
    ...
    private function _writeList($var, $spec, $output, $set = false)
    {
        ...
        $ewrite = null;
        if (isset(self::$tmethod[$etype])) {
            $ewrite = 'write'.self::$tmethod[$etype];
        } else {
            $espec = $spec['elem'];
        }
        if ($set) {
